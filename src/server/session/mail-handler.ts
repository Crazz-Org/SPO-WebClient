...
/**
 * One retry, and only on the deadline. MessageBody.asp is a legacy ASP page whose
 * first response is occasionally slower than the 5 s deadline; a single slow answer
 * used to leave the unread flag set for good, which shows up downstream as a
 * `CheckNewMail` count that never drops. A refused or erroring host is NOT retried —
 * that is a real answer, not latency.
 */
  ...
  for (let attempt = 1; attempt <= MAIL_READ_TOUCH_ATTEMPTS; attempt++) {
    try {
      ...
      if (e instanceof FetchTimeoutError && attempt < MAIL_READ_TOUCH_ATTEMPTS) {
        ctx.log.debug(`[Mail] Header touch timed out, retrying once: ${toErrorMessage(e)}`);
        continue;
      }
