...
function makeCtx(): FakeSessionCtx {
  ...
  fake.cacher.createObject.mockResolvedValue(DISCONNECT_TARGETS.tempObject); // substrate-exception: the fake's cacher emits no frame, so no RdoMock scenario can answer it
