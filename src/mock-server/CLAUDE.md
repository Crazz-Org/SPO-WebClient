...
Available scenarios: `auth`, `world-list`, `world-login`, `select-company`, `company-list`, `building-details`, `build-menu`, `build-roads`, `mail`, `switch-focus`, `civic-mutations`, `newspaper`, `connection-search`, `tycoon-profile`, `abandon-role`, `people-search`, `trade-settings`, `gate-map`, `product-owner`, `service-figures`, `bank-tv-live-reads`, `move-to`.
...
`move-to` is the one push-only scenario — the Interface Server's `MoveTo(x, y)`
(`Voyager/URLHandlers/ServerCnxHandler.pas:489`), fed through the real `dispatchPush` and the
real client `dispatchEvent`, proving a server-driven camera pan reaches the renderer and the
camera history.
