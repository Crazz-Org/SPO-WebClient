...
type Host = {
  ...
  gameObjectTextureCache: {
    getTextureSync: jest.Mock<(category: string, name: string) => ImageBitmap | null>;
    getAnimatedTexture: jest.Mock<(category: string, name: string) => AnimatedTexture | null>;
    getAnimatedFrame: jest.Mock<(animatedTexture: AnimatedTexture, elapsedMs: number) => ImageBitmap>;
  };
...
const staticBitmap = { width: 64, height: 64 } as unknown as ImageBitmap;
const animatedFrameBitmap = { width: 64, height: 64 } as unknown as ImageBitmap;
const animatedTexture = { frames: [], totalDuration: 500 } as unknown as AnimatedTexture;
...
function makeHost(overrides: Partial<Host> = {}): Host {
  return {
    ...
    gameObjectTextureCache: {
      getTextureSync: jest.fn<(category: string, name: string) => ImageBitmap | null>(() => staticBitmap),
      getAnimatedTexture: jest.fn<(category: string, name: string) => AnimatedTexture | null>(() => animatedTexture),
      getAnimatedFrame: jest.fn<(animatedTexture: AnimatedTexture, elapsedMs: number) => ImageBitmap>(() => animatedFrameBitmap),
    },
...
describe('drawBuildings — building animations switch', () => {
  it('draws the animated frame and marks hasAnimatedBuildings when the option is on', () => {
    const host = makeHost({
    ...
    expect(host.ctx.drawImage).toHaveBeenCalledWith(animatedFrameBitmap, expect.any(Number), expect.any(Number), expect.any(Number), expect.any(Number));
  ...
  it('draws the static frame and leaves hasAnimatedBuildings false when the option is off', () => {
    ...
    expect(host.ctx.drawImage).toHaveBeenCalledWith(staticBitmap, expect.any(Number), expect.any(Number), expect.any(Number), expect.any(Number));
