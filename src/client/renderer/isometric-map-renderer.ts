...
export class IsometricMapRenderer {
  ...
  /**
   * Legacy 'UseTransparency' (Map.pas:1415-1416): master on/off for fading every building
   * whose owner differs from the *selected* building's owner (Map.pas:5979, the clicked
   * building's owner) — no selection ⇒ nothing faded (Map.pas:1405-1406, `if ok`).
   */
  private glassForeignBuildings: boolean = true;
  /** The player's own tycoon id, 0 until login supplies one. Feeds only the losing-facilities reddening (:3598), not the glass fade. */
  ...
  /** The game store's decimal tycoon id; '' / undefined / unparsable means "unknown". Feeds only the losing-facilities reddening, not the glass fade. */
