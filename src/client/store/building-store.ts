...
interface ResearchState {
  ...
  /** Writes whose effect the block paints before the server read-back agrees (#888). */
...
export const useBuildingStore = create<BuildingState>((set) => ({
  ...
  setResearchInventory: (data) =>
    set((state) => {
      ...
      // Drop a pending op only once this authoritative read agrees with it —
      // the object cache can still serve the pre-write value for 30-90s
      // (OB-29), and dropping on mere arrival would bounce the item back out
      // of the block on the normal path.
