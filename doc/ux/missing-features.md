...
|---|---|---|---|---|
| N1 | **Vue « Carte » de données** (bâtiments par classe, mes bâtiments, déficitaires en rouge, brouillard sur l'inexploré, rectangle de vue, clic = sauter, zoom propre) | ✅ **Surface « Map » (Carte-1, PR #67)** — `components/map/MapSurface` dans la feuille (M, tuile Map, triangle mobile) : terrain (colormap partagée `ui/minimap-colormap.ts`), rectangle de vue, clic = sauter, molette/boutons = zoom 1–8× autour du curseur, glisser = déplacer. **Brouillard : fait (issue #600)** — blocs 64×64 jamais chargés assombris sur la vue iso et atténués sur la surface Map, `store/explored-blocks.ts`, persistant par monde et par joueur (`localStorage`). **Couleur par classe/zone, routes, béton et marqueur de sélection : faits (issue #601)** — chaîne de priorité portée de `TWorldMap.GetColor` (`ui/minimap-paint.ts`) : tuile sélectionnée en blanc, bâtiment mien déficitaire en rouge, sinon couleur de classe/zone (atténuée pour une autre compagnie), puis routes, puis béton. Le losange ancré reste (menu Plus › Docked minimap) | **V** `Map.pas:3512-3626` (couleur par classe `GetBuildingColor`, `cLoosingColor` rouge, `fSeen` → brouillard), `Map.pas:3855-3918` (chaîne complète), `IsometricMap.pas:438-451` (clic → `MoveTo`), `FiveIsometricMap.pas:12,44-62` (zoom 1–100 %) | — |
...
| N3 | **Aller à l'hôtel de ville le plus proche** | ✅ **fait sans RDO (Carte-1, PR #67 ; utilisable avant chargement + sélection à l'arrivée, issue #592)** — bouton « Nearest Town Hall » de la surface Map : utilisable même si l'annuaire n'est pas encore chargé (le clic le demande) ; distance de Manhattan sur les villes de la page annuaire ; arrivée via `onNavigateToBuilding` (centre **et** sélectionne, comme `MoveAndSelect`) | **V** `MapIsoView.pas:978-1007` (`GetNearestTownHall(x,y)` puis `MoveAndSelect`) | **Sans nouveau RDO** : la liste des villes (`search-menu-service.ts`, `TownInfo.x/y`) + distance de Manhattan côté client, approximation citée dans `src/shared/nearest-town.ts` (`Kernel/World.pas:5905-5933`, `:5921-5924`) |
...
| N5 | **Aller à des coordonnées** (palette « x,y », coordonnées cliquables dans le chat) | ✅ **fait (issue #617)** — Palette (T7, PR #64) : « x,y » / « x y » → `Go to (x, y)` via `onNavigateToBuilding` ; palette atteignable sur mobile (PR #70, pastille de recherche). Chat : la commande de déplacement du tchat (`x,y`) déplace la caméra (`src/client/chat-commands.ts`) et les coordonnées d'une ligne reçue sont cliquables (linkification), affordance moderne sans citation Voyager | **V** URL `MoveTo`/`Select x,y` (`MapIsoHandler.pas:240-254`), commande de déplacement du tchat (`ChatHandler.pas:88-132`) | — |
...
| N9 | **« Voir sur la carte »** depuis l'inspecteur | ✅ **fait (T3, issue #877)** — bouton « View on map » sur la ligne de pile de la feuille, à côté de Pin et Close (`Sheet.tsx`, `BuildingSheetActions.tsx`) ; avant : 🟡 recentrage implicite via `focusBuilding` ; **aucun bouton** explicite | **V** `MoveAndSelect` depuis les fiches (`ProdSheetForm.pas:761`) | Bouton dans le pied de la feuille |
