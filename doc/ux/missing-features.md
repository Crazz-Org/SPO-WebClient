...
|---|---|---|---|---|
| N1 | **Vue « Carte » de données** (bâtiments par classe, mes bâtiments, déficitaires en rouge, brouillard sur l'inexploré, rectangle de vue, clic = sauter, zoom propre) | 🟡 **Surface « Map » (Carte-1, PR #67 ; couleur par classe/zone, routes, béton, sélection — #601)** — `components/map/MapSurface` dans la feuille (M, tuile Map, triangle mobile) : terrain (colormap partagée `ui/minimap-colormap.ts`), **tous les bâtiments chargés** (couleur par zone de classe, miens vifs / autres atténués, mes déficitaires en rouge, routes et béton, tuile sélectionnée — #601), rectangle de vue, clic = sauter, molette/boutons = zoom 1–8× autour du curseur, glisser = déplacer. Pas de brouillard (l'inexploré n'est pas suivi côté client). Le losange ancré reste (menu Plus › Docked minimap) | **V** `Map.pas:3512-3626,3732-3778,6930-6950` (couleur par classe `GetBuildingColor`, `cLoosingColor` rouge, `fSeen` → brouillard), `IsometricMap.pas:438-451` (clic → `MoveTo`), `FiveIsometricMap.pas:12,44-62` (zoom 1–100 %) | Brouillard sur la minicarte ; zoom molette sur la minicarte ; feuille « Carte » desktop + mobile |
